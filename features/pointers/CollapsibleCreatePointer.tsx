...
const PIN_STORAGE_KEY = "gestion-personas:punteros:formAbierto";
...
export function CollapsibleCreatePointer({
  canWrite,
  onOpenChange,
  closeSignal,
  openSignal,
  ...
  // Patron "ajustar estado durante el render" (en vez de useEffect):
  ...
  // afuera -- lo usa el botón "Agregar" que aparece cuando la lista esta
  // vacia (ver PointersList), asi no hay que buscar el boton de arriba.
  ...
  if (!canWrite) {
    return (
      <p className="flex items-start gap-2 rounded-lg bg-zinc-100 p-3 text-sm text-zinc-600">
        ...
        La carga está cerrada en este momento. Podés consultar tus punteros, pero no agregar
        ni modificar nada.
      </p>
    );
  }
    ...
    <div className="flex flex-col gap-2">
      {showSuccess ? (
        ...
        >
          ...
          El puntero fue creado exitosamente.
        </p>
      ) : null}
      ...
      {!open ? (
        ...
        <div className="flex flex-col gap-3 rounded-xl border-2 border-zinc-300 bg-white p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-medium text-zinc-900">Agregar puntero</h2>
            <button
              type="button"
          ...
          <CreatePointerForm key={formKey} onCreated={handleCreated} />
